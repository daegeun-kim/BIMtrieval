...
- Codex's default writable scope is Markdown documentation only, primarily `specs/*.md`, `tasks/*.md`, `logs/*.md`, `readme.md`, `workflow.md`, and `CODEX.md` when I explicitly ask to update instructions.
...
- only generate md files in C:\Users\kdgki\Desktop\MSCDP\Projects\BIM_RAG\specs, C:\Users\kdgki\Desktop\MSCDP\Projects\BIM_RAG\tasks, or C:\Users\kdgki\Desktop\MSCDP\Projects\BIM_RAG\logs when I explicitly tell you to do so.
...
- Keep `specs/` limited to authoritative project blueprints. Test-query reports, evaluation logs, debugging instructions, per-iteration records, and shared evaluation references belong in `logs/`.
- Files in `logs/` should use descriptive names. Automated v5 debugging iterations use `logs/debug_v5_iteration_NNN.md` with zero-padded sequential numbers and must never overwrite an earlier iteration.
