"""SQLAlchemy ORM models for the BIM RAG pipeline.

Stage 1 tables: ifc_source_models, ifc_entities, ifc_relationships, relationship_members
Stage 2 table:  rag_documents  (requires pgvector extension)
ElementVector is kept for migration-safety checks but is not created by any active path.
"""
...
class IfcSourceModel(Base):
    ...
    entities = relationship(
        "IfcEntity", back_populates="source_model", cascade="all, delete-orphan"
    )
...
class IfcEntity(Base):
    ...
    vectors = relationship("ElementVector", back_populates="entity", cascade="all, delete-orphan")
...
# ---------------------------------------------------------------------------
# Stage 2 (requires CREATE EXTENSION vector before table creation)
# ---------------------------------------------------------------------------


class ElementVector(Base):
    """One element_description embedding per eligible IFC entity."""

    __tablename__ = "element_vectors"
    __allow_unmapped__ = True

    id = Column(Integer, primary_key=True, autoincrement=True)
    entity_id = Column(Integer, ForeignKey("ifc_entities.id", ondelete="CASCADE"), nullable=False)
    document_type = Column(Text, nullable=False, default="element_description")
    document_text = Column(Text, nullable=False)
    text_truncated = Column(Boolean, nullable=False, default=False)
    text_template_version = Column(Text, nullable=False, default="v001")
    embedding_model = Column(Text, nullable=False, default="BAAI/bge-m3")
    ...
    entity = relationship("IfcEntity", back_populates="vectors")
    ...
    __table_args__ = (
        UniqueConstraint(
            ...
            "embedding_model",
            name="uq_vector_entity_type_version",
        ),
        CheckConstraint("document_type = 'element_description'", name="ck_document_type"),
        CheckConstraint("embedding_dim = 1024", name="ck_embedding_dim"),
        Index("ix_element_vectors_entity_id", "entity_id"),
    )


# ---------------------------------------------------------------------------
# Stage 2 (unified): rag_documents (requires pgvector)
# ---------------------------------------------------------------------------
...
class RagDocument(Base):
    ...
    __table_args__ = (
        ...
        # HNSW index for fast cosine similarity search (added after data load)
        Index(
...
# ---------------------------------------------------------------------------
# Stage 1 (extension): Relationship tables
# ---------------------------------------------------------------------------
...
class DbIfcRelationship(Base):
    ...
    members = relationship(
        "RelationshipMember", back_populates="ifc_relationship", cascade="all, delete-orphan"
    )
