...
from config.logging import write_jsonl_event
from config.settings import Settings, get_settings
from db.session import session_scope
from shared.errors import BimRagError, ModelNotFoundError
...
from llm.answerer import answer_from_evidence, answer_general
from llm.client import LLMError, LLMUnavailableError, OpenAIQueryClient, get_llm_client
from llm.context import build_planner_context
from llm.prompts import ANSWERER_PROMPT_VERSION, PLANNER_PROMPT_VERSION
from llm.schemas import QueryPlan
from llm.translate import TranslatedPlan, translate_plan
from llm.validation import PlanValidationError, validate_plan_structure
from query.hybrid.evidence import apply_bounds
from query.hybrid.orchestrator import orchestrate
from query.hybrid.schemas import EvidencePackage
from query.rag.embedding_service import get_embedding_service
from query.session import SessionState, get_session_store
from query.sql import catalog as catalog_ops
from query.sql.schemas import GetModelMetadataPlan
...
class QueryService:
    ...
    def _handle_confirmation(
        ...
        try:
            with session_scope() as session:
                ...
                viewer_source = getattr(row, "viewer_source_location", None)
    ...
    def _handle_question(
        ...
        try:
            with session_scope() as session:
                context = build_planner_context(session, request, state, self.settings)
                plan, translated, repaired, final_errors = self._plan_and_translate(
                    client, context, session, request.selected_entity_ids
                )
                planner_ms = round((time.perf_counter() - t0) * 1000.0, 1)

                if translated is None:
                    return self._clarify_after_repair(
                        request, request_id, scope, plan, final_errors, client, t0
                    )

                envelope = self._execute_and_answer(
                    request, request_id, session, plan, translated, client, state, t0, repaired,
                    planner_ms,
                )
                return envelope
    ...
    def _execute_and_answer(
        ...
        self._finalize_state(state, plan, pkg)
        self._log_event(
            request, request_id, plan, pkg, client, t0, repaired,
            ans.output.used_general_knowledge, stages,
        )
        return _from_package(request, request_id, plan, pkg, ans.output.answer, viewer)
    ...
    def _finalize_state(
        self, state: SessionState, plan: QueryPlan, pkg: EvidencePackage
    ) -> None:
    ...
    def _log_event(
        ...
        record = {
            ...
            "path_runs": [
                {"name": p.name, "ok": p.ok, "error": p.error} for p in pkg.path_runs
            ],
