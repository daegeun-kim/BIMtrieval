...
into `.env` as `DATABASE_URL=...`, which `config.settings.Settings.get_database_url()`
(backend/src/config/settings.py) already prefers over the ingestion `db_url`.

Run manually (idempotent — safe to re-run):
    python -m db.bootstrap_readonly_role
"""
