...
from llm.schemas import CatalogPlan, PlanFieldRef, PlanFilter, QueryPlan, SqlPlan
from llm.validation import PlanValidationError
from query.sql.errors import AmbiguousFieldError, FieldNotFoundError
from query.sql.field_registry import resolve_field
...
def _build_entity_condition(
    ...
    try:
        ...
        raise PlanValidationError(
            f"field {pf.field.field_name!r} is ambiguous; specify field_kind and set_name "
            f"({exc})"
        ) from None
    except FieldNotFoundError as exc:
        raise PlanValidationError(
...
def _catalog_filter_group(filters: list[PlanFilter], bool_op: str) -> FilterGroup | None:
    ...
    from query.sql.catalog import CATALOG_FILTER_FIELDS, _CATALOG_BOOLEAN_COLUMNS
...
def _translate_sql(session: Session, source_model_id: int, plan: SqlPlan) -> tuple[SqlOperation, Any]:
    op = plan.operation
