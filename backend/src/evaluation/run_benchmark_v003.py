...
Usage (from repo root, bim_rag env):
    python -m evaluation.run_benchmark_v003            # needs backend/src on path
or
    python backend/src/evaluation/run_benchmark_v003.py
"""
...
# Allow running as a plain script (python backend/src/evaluation/run_benchmark_v003.py)
...
from api.schemas.request import HistoryTurn, SessionQueryRequest  # noqa: E402
from config.settings import get_settings  # noqa: E402
from db.session import session_scope  # noqa: E402
from llm.client import get_llm_client  # noqa: E402
from llm.context import build_planner_context  # noqa: E402
from query.service import QueryService  # noqa: E402
from query.session import get_session_store  # noqa: E402

CASES_PATH = _SRC / "evaluation" / "benchmark_v003_e2e_cases.jsonl"
RESULTS_PATH = Path("backend/logs/benchmark_v003_results.json")
    ...
    if case.get("expected_operations") and operation is not None:
        ...
        if not checks["operation_ok"]:
            result["issues"].append(
                f"operation {operation} not in {case['expected_operations']}"
            )
...
def _summarize(results: list[dict], before: dict, after: dict, wall_s: float) -> dict:
    ...
    for k in stage_keys:
        vs = [
            r["stage_latency_ms"].get(k, 0.0)
            for r in results
            if r.get("stage_latency_ms")
        ]
