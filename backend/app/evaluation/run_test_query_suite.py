...
class Outcome:
    ...
    statuses: str
    used_fallback: bool
    #: Request cost in USD from the versioned pricing registry (task25 §6.1),
    #: or None when a call's model/tier is not on the recorded rate card.
    cost_usd: float | None = None
    cost_note: str | None = None
    #: Tokens billed against the rate-limited binder/correction model, for pacing.
    paced_model_tokens: int = 0
    error: str | None = None


class _TpmPacer:
    """Keep binder/correction calls under the model's tokens-per-minute limit.

    The cheap binder model (gpt-5.4-nano) has a 200k TPM ceiling, and one model-2
    binder call is ~99k tokens, so two consecutive cases would 429. This tracks
    tokens billed against that model in a rolling 60-second window and sleeps
    before a case only when the next one would breach the budget. It is a harness
    concern only — the production pipeline never sleeps.
    """

    _WINDOW_S = 60.0
    #: Headroom for a WORST-CASE next case: a cold binder plus a correction call
    #: is ~2x a single call. Sizing the reserve for that avoids the 429s seen when
    #: a correction unexpectedly doubled a case's nano usage.
    _RESERVE = 150_000
    ...
    def _tokens_in_window(self, now: float) -> int:
        self._events = [(t, n) for t, n in self._events if now - t < self._WINDOW_S]
    ...
    def wait_if_needed(self) -> None:
        ...
        while True:
            ...
            if used + self._RESERVE <= self.limit or not self._events:
                return
            oldest = min(t for t, _ in self._events)
            sleep_s = max(1.0, self._WINDOW_S - (now - oldest) + 0.5)
            print(
                f"    (pacing: {used:,} tok used in the last minute; "
                f"waiting {sleep_s:.0f}s to stay under {self.limit:,} TPM)",
                flush=True,
            )
            time.sleep(sleep_s)
...
def _request_cost(calls: list[dict]) -> tuple[float | None, str | None]:
    """Sum per-call USD cost from the versioned pricing registry (task25 §6.1).

    Each call is costed against its OWN model, so a mixed-model request (a Sol
    binder plus a Terra answer, once Task 25 lands) totals correctly. A model
    absent from the rate card makes the request cost unavailable rather than
    silently zero.
    """
    from app.llm.pricing import cost_for_call, cost_for_request, cost_from_simple_usage

    per_call = []
    for c in calls:
        model = c.get("model", "")
        if "uncached_input_tokens" in c:
            # Responses API: use the exact mutually-exclusive billable buckets so
            # cached and cache-write tokens are priced at their own rates.
            per_call.append(
                cost_for_call(
                    model=model,
                    uncached_input_tokens=int(c.get("uncached_input_tokens", 0) or 0),
                    cached_input_tokens=int(c.get("cached_input_tokens", 0) or 0),
                    cache_write_tokens=int(c.get("cache_write_tokens", 0) or 0),
            ...
            per_call.append(
                cost_from_simple_usage(
                    model,
                    int(c.get("prompt_tokens", 0) or 0),
                    int(c.get("completion_tokens", 0) or 0),
                )
            )
    total = cost_for_request(per_call)
    if total is None:
        return None, "no LLM calls"
    if not total.available:
        return None, total.unavailable_reason
    return total.usd, None
...
def _format_case(outcome: Outcome) -> str:
    ...
    cost = (
        ...
        else f"cost unavailable ({outcome.cost_note})"
    )
    metrics = (
        ...
        f"{outcome.completion_tokens}c · cost={cost} · db={outcome.db_statements} · "
...
def _v3_header(run_date: str, planner: str, answerer: str) -> list[str]:
    ...
    def _rate_line(role: str, model: str, effort: str) -> str:
        rates = get_rates(model)
        if rates is None:
            return f"- {role}: `{model}` ({effort} reasoning) — rate not on the recorded card"
        return (
            f"- {role}: `{model}` ({effort} reasoning) — ${rates.uncached_input:g} / 1M input, "
            f"${rates.cached_input:g} cached, ${rates.cache_write:g} cache-write, "
            f"${rates.output:g} / 1M output"
        )
    ...
    return [
        ...
        f"(task25 §6.1, registry `{PRICING_REGISTRY_VERSION}`, rates from <{PRICING_SOURCE_URL}>),",
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=int,
        default=200_000,
        help="binder/correction model tokens-per-minute limit to pace under (0 disables)",
    )
    ...
    for index, case in enumerate(cases, start=1):
        ...
        print(
            ...
            f"count={outcome.count} · highlighted={outcome.highlighted} · cost={cost}"
    ...
    total_cost = f"${sum(priced):.6f}" if priced else "n/a"
    print(
        f"\ncases={len(outcomes)} median_latency={median} ms "
        f"tokens={total_prompt}p/{total_completion}c "
        f"cost={total_cost} (priced {len(priced)}/{len(outcomes)}) "
        f"fallbacks={sum(1 for o in outcomes if o.used_fallback)} "
        f"errors={sum(1 for o in outcomes if o.error)}"
    )
    print("\nper-query cost:")
    for o in outcomes:
        c = f"${o.cost_usd:.6f}" if o.cost_usd is not None else f"unavailable ({o.cost_note})"
        print(f"  {o.case.case_id}: {c}  (count={o.count}, calls={o.llm_calls})")
    return 0
