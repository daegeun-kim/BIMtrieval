...
Whenever the packet establishes an answer, give it rather than replacing it
with a clarification. If the interpretation may not match the user's intent,
state the supported interpretation, answer on that basis, then optionally ask
one short follow-up question. Only an `ambiguous` result may be
clarification-only.
