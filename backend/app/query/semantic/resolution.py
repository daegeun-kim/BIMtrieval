...
class FacetResolution:
    """Stage-3 semantic resolution for ONE conceptual facet (Task 17 §1 Stage 3).

    Populated AFTER the retrieval policy is frozen; it can never see or mutate the
    policy. It only answers "how might this facet be represented in the model?"."""
...
def _build_candidates(
    ...
    settings: Settings,
) -> tuple[list[OntologyCandidate], list[ModelClassCandidate], list[ModelFactCandidate]]:
    """Shared threshold-free top-k candidate construction for one query vector."""
    onto: list[OntologyCandidate] = []
    if onto_index is not None:
        for i, sim in _cosine_topk(
            onto_index.vectors, query_vec, settings.semantic_resolution_top_k
        ):
            e = onto_index.entities[i]
    ...
    classes: list[ModelClassCandidate] = []
    facts: list[ModelFactCandidate] = []
    class_k = settings.semantic_resolution_model_top_k
    fact_k = settings.semantic_resolution_model_top_k
    seen_facts: set[tuple] = set()
    for i, sim in _cosine_topk(model_index.vectors, query_vec, (class_k + fact_k) * 4):
        item = model_index.items[i]
        if item.kind == "class" and len(classes) < class_k:
            c: ClassProfile = item.ref
            classes.append(
                ModelClassCandidate(
                    ifc_class=c.ifc_class,
        ...
        elif item.kind == "fact" and len(facts) < fact_k:
            f: ObservedFactProfile = item.ref
            if f.fact_kind == "property_coverage":
                continue
            key = (f.ifc_class, f.fact_kind, f.observed_value)
        ...
        if len(classes) >= class_k and len(facts) >= fact_k:
            break
    return onto, classes, facts
...
def resolve_facets(
    ...
    *,
    ...
    """Stage 3: resolve each conceptual facet against the ontology + model
    vocabulary (Task 17 §1). This runs strictly AFTER the retrieval policy is
    frozen and returns candidates/definitions only — it never returns a modality
    decision. On embedding failure every facet degrades to empty candidates so
    the fixed SQL policy stays usable (§2)."""
    settings = settings or get_settings()
    out: list[FacetResolution] = []
    vocab = get_model_vocabulary(session, source_model_id, settings)
    present = vocab.present_classes()
    ...
    for f in facets:
        query_vec = service.embed_query(f.semantic_query)
        onto, classes, fact_cands = _build_candidates(
            query_vec, onto_index, model_index, present, vocab, settings
        )
        out.append(
            FacetResolution(
                facet_id=f.facet_id,
                role_hint=f.role_hint.value,
                semantic_query=f.semantic_query,
                ontology_candidates=onto,
                model_class_candidates=classes,
                model_fact_candidates=fact_cands,
            )
        )
    return out
