...
from app.query.hybrid import combination as combine
from app.query.hybrid.concurrency import run_parallel
from app.query.hybrid.evidence import fetch_context, fetch_primary, fetch_relationships
from app.query.hybrid.schemas import EvidencePackage, PathRun, RagInternalItem
from app.query.rag.hydration import hydrate_rag_result
from app.query.rag.search import run_rag_search
from app.query.sql.dispatch import execute_catalog, execute_sql
from app.shared.errors import DegradedCapabilityError
from app.shared.types import AnswerBasis, QueryRoute
...
def _run_rag_route(
    ...
    pkg.answer_basis = (
        AnswerBasis.SEMANTIC_RETRIEVAL
        if rag_res.sufficient_evidence
...
def _relationship_expansion(session, plan, sql_ids, rag_rel_ids, rag_res):
    ...
    return combine.CombinationOutcome(
        primary_ids=primary, context_ids=context, groups=groups
    ), rag_rel_ids
